...
describe('getDifferenceInDays', function() {
  it('returns 1 for consecutive days', function() {
    const result = getDifferenceInDays(
      { day: 1, month: 4, year: 2000 },
      { day: 2, month: 4, year: 2000 }
    );
    eq(result, 1);
  });
  ...
  it('returns the same result regardless of argument order', function() {
    const result1 = getDifferenceInDays(
      { day: 1, month: 10, year: 2000 },
      { day: 1, month: 4, year: 2000 }
    );
    const result2 = getDifferenceInDays(
      { day: 1, month: 4, year: 2000 },
      { day: 1, month: 10, year: 2000 }
    );
    eq(result1, result2);
  ...
  it('returns days across a year', function() {
    const result = getDifferenceInDays(
      ...
      { day: 31, month: 12, year: 2001 }
    );
    eq(result, 364);

    const leapYear = getDifferenceInDays(
