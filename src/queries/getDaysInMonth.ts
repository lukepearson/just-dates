...
import { YearMonth, DateObject, StringDateObject } from '../dateObject';
 ...
 * // 29
 */
export const getDaysInMonth = (date: YearMonth | DateObject | StringDateObject): number => {
  [date] = stringToNumber([{ day: 1, ...date }]);
  checkArgs(date, 'date');
  let { year, month } = date;
  while (month < 1) {
    month += 12;
